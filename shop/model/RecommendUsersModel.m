...
@implementation RecommendUsersModel

DEF_SINGLETON( RecommendUsersModel )
...
- (void)loadCache
{
	NSString * string = [self userDefaultsRead:@"RecommendUsersModel.regions"];
	if ( string )
	{
...
- (void)saveCache
{
	...
	if ( string )
	{
		[self userDefaultsWrite:string forKey:@"RecommendUsersModel.regions"];
	}
}
...
{
	self.CANCEL_MSG( API.recommendUsers );
	self.MSG( API.recommendUsers )
